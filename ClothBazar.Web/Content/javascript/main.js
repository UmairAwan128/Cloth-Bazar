...
;(function($) {
   ...
   'use strict'
    ...
    //this code was for the slider of the price we removed it and write our own in the index.cshtml of Shop 
    //controller at the end of the file
    ...
    $(function () {
      ...
      flatCounter();
      googleMap();
      //flatPrice();   //we commented the Jquery function also comment its calling 
      flatFilterBox(); 
      flatShopSearch();
      topSearch();
      flexProduct(); 
      ...
      //we called our updateCartProducts() function here in this annonymous function which will be auto
      //called when the dom is ready i.e all html/design is shown to user we called it here because as after
      //adding products into cart in Shop page we were calling updateCartProducts() there so the value cart icon/badge
      // value on the rigt top corner updates but when we move to any other page like Home/Checkout this cart icon/badge
        // value is not the same but default so to update it every where we called updateCartProducts() here in this
        //annonymous function which will be auto called for each page as each page has included main.js file.
      updateCartProducts();
    });
...
//the following are our own created functions it show the loader when we call we called it in Index.cshtml of Shop controller
// in the GetData(dUrl) as there on the ajax call we get products so if that call takes time this method will show loader.
function showLoader() {
    $(".loader").show();
    $("#loading-overlay").show();
};
//after ajax call completes hide the loader and show the data
function hideLoader() {
    $(".loader").hide();
    $("#loading-overlay").hide();
};
//this method will update the count of total products in the cart this is showing on the top right cart icon/badge
function updateCartProducts() {
    var totalCartProducts; //created a Products var this will be having all the procucts adds to cart like array
    ...
    if (existingCookieData != null && existingCookieData != "" && existingCookieData != undefined) {
        totalCartProducts = existingCookieData.split('-'); //then store them by splititng on basis of -
    }
    else {
        totalCartProducts = [];  //if null the initialize it an empty array so that we can push data
    }
    $("#cartProductsCount").html(totalCartProducts.length);
};
